...
#         self.min = min
    ...
    def controller_out(self):
        ...
        # self.rc_th.append(userRC[self.index].rcThrottle)
        # self.rc_r.append(userRC[self.index].rcRoll)
        # self.rc_p.append(userRC[self.index].rcPitch)
        # self.rc_y.append(userRC[self.index].rcYaw)
        # self.sen_r.append(userRC[self.index].orientation_x)
        # self.sen_p.append(userRC[self.index].orientation_y)
        # self.sen_y.append(relative_yaw)
        # self.cam_x.append(userRC[self.index].current_x)
        # self.cam_y.append(userRC[self.index].current_y)
        # self.cam_z.append(userRC[self.index].current_z)
...
def aruco_detect():
    ...
    while cap.isOpened():
        ...
        for index in range(len(id)):
            if ids is not None and [id[index]] in ids:
                #-- ret = [rvec, tvec, ?]
                ...
                # # ids = ids.tolist()
                # print(type(ids))
                pos = np.where(ids == [id[index]])[0]
                # pos = ids.index([id[index]])

                print(corners)
                print(tuple(np.array([[corners[0][pos][0]]])))
                corners_index = tuple(np.array([[corners[0][pos][0]]]))


                ret = aruco.estimatePoseSingleMarkers(corners_index, marker_size, camera_matrix, camera_distortion)
...
def rectangle():
    ...
    # while (not plutoPID[0].reached_target) or (userRC[0].current_z > 200):
    ...
    #     plutoPID[0].q_pitch[i]=0
    #     plutoPID[0].q_yaw[i]=0
    # print("Reached target:", plutoPID[0].reached_target)
    # print("Reached target:", plutoPID[0].reached_target)
    # print("Reached target:", plutoPID[0].reached_target)
    # print("Reached target:", plutoPID[0].reached_target)
...
if __name__ == '__main__':  
    ...
    for i in range(len(mul_clients)):
        ...
        if i==1:
            plutoPID.append(PID(K_z,K_roll,K_pitch,K_yaw,dt=0.1,tau=0.06,alpha = 0.5,cam_orientation= 30, tolerance=15, index = i, beta=0.5))
    ...
    plutoPID[0].set_target(-40,40,150)  
    plutoPID[1].set_target(40,40,150)
